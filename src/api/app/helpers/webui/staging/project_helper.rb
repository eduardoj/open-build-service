module Webui::Staging::ProjectHelper
  def icon_for_checks(checks, missing_checks)
    ...
    return 'fa-check-circle link-primary' if checks.blank?
    return 'fa-eye text-info' if checks.any?(&:pending?)
    return 'fa-check-circle link-primary' if checks.all?(&:success?)

    'fa-exclamation-circle text-danger'
  end

  def icon_for_check(check)
    return 'fa-check-circle link-primary' if check.success?
